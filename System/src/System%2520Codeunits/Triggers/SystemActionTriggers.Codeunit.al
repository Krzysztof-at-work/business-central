...
codeunit 2000000006 "System Action Triggers"
{
    ...
    /// <summary>
    /// Event triggered when feedback is requested on a page.
    /// </summary>
    [Scope('OnPrem')]
    [BusinessEvent(false)]
    procedure OnFeedbackEvent(PageId: Integer; Context: Dictionary of [Text, Text]; var Handled: Boolean)
    begin
    end;

    /// <summary>
    /// Gets the execution context for agent tasks evaluations, which includes information about the current state and limits for AI evaluations.
    /// </summary>
    [Scope('OnPrem')]
    [BusinessEvent(false)]
    procedure GetAgentTaskEvalExecutionContext(AgentUserSecurityId: Guid; TaskId: BigInteger; var Context: JsonObject)
    begin
    end;
}